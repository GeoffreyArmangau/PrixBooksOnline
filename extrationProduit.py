...
#url à parser pour une catégorie
urlcategorie = 'https://books.toscrape.com/catalogue/category/books/mystery_3/index.html'

#saisir l'url
rcategorie = requests.get(urlcategorie)
rcategorie.encoding='utf-8'

soupCategorie = BeautifulSoup(rcategorie.text, 'html.parser')

# lister les livres dans une catégorie
categories = soupCategorie.find_all('h3')
urlLivres = []

#scrapper les url dans une catégorie
for livres in categories:
    urlLivre = livres.find('a')['href']
    urlLivre = urljoin('https://books.toscrape.com/catalogue/', urlLivre)
    urlLivres.append(urlLivre)    
#print(urlLivres)

# boucler sur les url des livres pour les scrapper
